...
class searchnet:
	...
	def generatehiddennode(self, wordids, urls):
		...
		if res == None:
			...
			for wordid in wordids:
				self.setstrength(wordid, hiddenid, 0, 0.5)
			for urlid in urls:
	...
	def getresult(self, word):
		urlids = [0,1]
		wordids = self.assign(self.split(word))
	...
	def train(self,word,ans):
		wordids = self.assign(self.split(word))
		self.setupnetwork(wordids,[0,1])
		self.trainquery(wordids,[0,1],ans)
	...
	def split(self,l):
		...
		for i in l:
			if i==" ":
				list.append(l[ind:ctr])
				ind = ctr+1
			ctr = ctr + 1
		return list
	
	def assign(self,h):
		con = sqlite.connect("ids.db")
		ind = 0
		ctr = 0
		l = []
		i = ""
		for i in h:
					res = con.execute("select rowid from wordids where word='"+i+"'").fetchone()
					if res==None:
						con.execute("insert into wordids(word) values ('"+i+"')")
						l.append(con.execute("select rowid from wordids where word='"+i+"'").fetchone()[0])
					else: l.append(res[0])
		con.commit()
		return l